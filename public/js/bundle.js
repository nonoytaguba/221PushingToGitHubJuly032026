...
const VERSION = exports.VERSION = "1.18.0";
...
const showAlert = (type, msg) => {
  ...
  document.querySelector('body').insertAdjacentHTML('afterbegin', markup); //Lesson 191
...
const login = async (email, password) => {
  ...
  try {
    ...
    if (res.data.status === 'success') {
      ...
      window.setTimeout(() => {
        // let's then after one and a half seconds load the front page. So basically, the home page. Lesson 190
        location.assign('/');
      }, 1500);
    }
    // console.log(res);
...
const logout = async () => {
  try {
    ...
    (0, _alerts.showAlert)('error', 'Error Log logging out! try again.');
  }
};
...
if (userDataForm) userDataForm.addEventListener('submit', e => {
  ...
  (0, _updateSettings.updateSettings)(form, 'data'); //Lesson 203
});
